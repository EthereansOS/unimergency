    ...
    return new Promise(function(ok) {
        setTimeout(async function() {
            ...
            if (!window.networkId || window.networkId !== await window.web3.eth.net.getId()) {
                ...
                window.arte = window.newContract(window.context.ERC721ABI, window.getNetworkElement("arteTokenAddress"));
                window.etra = window.newContract(window.context.ERC721ABI, window.getNetworkElement("etraTokenAddress"));
                window.contest = window.newContract(window.context.contestV2Abi, window.getNetworkElement("contestV2Address"));
                window.nft = window.newContract(window.context.ERC721ABI, await window.blockchainCall(window.contest.methods.nftTokenAddress));
                window.nftTicker = await window.blockchainCall(nft.methods.symbol);
                window.votingToken = window.newContract(window.context.IERC20Abi, await window.blockchainCall(window.contest.methods.votingTokenAddress));
...
window.loadTokenExclusions = function loadTokenExclusions() {
    ...
    window.voteExclusionsByToken[window.web3.utils.toChecksumAddress(window.arte.options.address)] = window.voteExclusionsByToken.arte;
    window.voteExclusionsByToken[window.web3.utils.toChecksumAddress(window.etra.options.address)] = window.voteExclusionsByToken.etra;
    delete window.voteExclusionsByToken.arte;
    delete window.voteExclusionsByToken.etra;
};
...
window.getAddress = async function getAddress() {
    ...
    return window.web3.utils.toChecksumAddress(window.walletAddress = (await window.web3.eth.getAccounts())[0]);
};
...
window.renderItemsForContest = function renderItemsForContest(callback) {
    return window.renderItemsPromise = window.renderItemsPromise || new Promise(async function(ok) {
...
window.renderItemsOfToken = async function renderItemsOfToken(token, ticker, tokenId, items, callback) {
